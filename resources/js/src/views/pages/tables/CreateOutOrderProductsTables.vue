       ...
       <template v-for="(recipe, index) in recipes"> 
         ...
         <td class="px-2 ">
           ...
           <td class="text-center px-2"> 
             {{  
              Number.isInteger((parseFloat(recipe.pivot.quantity) * parseFloat(product.pivot.quantity)))  
              ? (parseFloat(recipe.pivot.quantity) * parseFloat(product.pivot.quantity)).toFixed(0) 
